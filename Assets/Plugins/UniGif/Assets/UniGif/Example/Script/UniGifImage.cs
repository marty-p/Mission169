...
public class UniGifImage : MonoBehaviour
{
    ...
    private IEnumerator GifLoopCoroutine()
    {
        ...
        {
            UnityEngine.Debug.LogError("TargetRenderer or GIF texture is nothing.");
            yield break;
        }
        // play start
        state = State.Playing;
        ...
        do
        {
            for (int i = 0; i < m_gifTexList.Count; i++)
            {
                ...
                float delayedTime = timeSince + 1/15f;
                //float delayedTime = Time.time + m_gifTexList[i].m_delaySec;

                while (delayedTime > (float) startTime.Elapsed.TotalSeconds)
                {
                    yield return 0;
                }
                // Pause
    ...
    public void Stop()
    {
        ...
        {
            UnityEngine.Debug.Log("State is not PLAYING and PAUSE.");
            return;
        }
        StopAllCoroutines();
        state = State.Ready;
    ...
    public void Pause()
    {
        ...
        {
            UnityEngine.Debug.Log("State is not PLAYING.");
            return;
        }
    ...
    public void Resume()
    {
        ...
        {
            UnityEngine.Debug.Log("State is not PAUSE.");
            return;
        }
